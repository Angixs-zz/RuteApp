...
export default function AdminViajeDetalle() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      api.get(`/actividades/viaje/${viajeId}`),
      api.get(`/gastos/viaje/${viajeId}`),
    ]).then(
      ([
        viajeResultado,
        ...
        if (viajeResultado.status === 'rejected') {
          setError('No fue posible cargar el viaje.');
        } else {
          setViaje(viajeResultado.value.data);
        }

        if (participantesResultado.status === 'fulfilled') {
          setParticipantes(participantesResultado.value.data ?? []);
        }

        if (actividadesResultado.status === 'fulfilled') {
          setActividades(actividadesResultado.value.data ?? []);
        }

        if (gastosResultado.status === 'fulfilled') {
          setGastos(gastosResultado.value.data ?? []);
        }
  ...
  const eliminarViaje = async () => {
    try {
      ...
      navegar('/admin/viajes', { replace: true });
    } catch {
  ...
  const rangoFechas = viaje?.fechaInicio
    ? `${new Date(`${viaje.fechaInicio}T00:00:00`).toLocaleDateString(
        'es-MX',
        { dateStyle: 'medium' },
      )} - ${
        viaje.fechaFin
          ? new Date(`${viaje.fechaFin}T00:00:00`).toLocaleDateString(
              'es-MX',
              { dateStyle: 'medium' },
            )
          : 'sin fecha final'
    ...
    : 'Fechas por definir';
  const [etiquetaEstado, claseEstado] = detallesPorEstado[viaje?.estado] ?? [
    viaje?.estado || 'Sin estado',
    'planning',
  ];
  const totalGastos = gastos.reduce(
    (totalAcumulado, gasto) =>
      totalAcumulado + Number(gasto.monto || 0),
    0,
  );
  ...
  return (
    <>
              ...
              <>
                <section className="trip-hero">
                  <div className="trip-hero-content">
                    <div>
                      ...
                      <h1>{viaje.nombre}</h1>
                      <p>
                        {viaje.destino || 'Sin destino'} · {rangoFechas}
                      </p>
                    </div>
                    <div className="form-actions admin-page-actions">
                      <Link className="button ghost" to="/admin/viajes">
                        Volver
                      </Link>
                      <Link
                ...
                <section className="content-card">
                  ...
                  <div className="info-grid">
                    ...
                    <div className="info-item">
                      <span>Origen</span>
                      <strong>{viaje.origen || 'Sin origen'}</strong>
                    </div>
                    <div className="info-item">
                      <span>Destino</span>
                      <strong>{viaje.destino || 'Sin destino'}</strong>
                    </div>
                    <div className="info-item">
                      <span>Transporte</span>
                      <strong>{viaje.transporte || 'Sin definir'}</strong>
                    </div>
                    <div className="info-item">
                    ...
                    <div className="info-item">
                      ...
                      <strong>${totalGastos.toLocaleString('es-MX')}</strong>
                    </div>
                    <div className="info-item">
                      <span>Visibilidad</span>
                      <strong>
                        {viaje.publico ? 'Público' : 'Privado'}
                      </strong>
                    </div>
                  </div>
                  {viaje.descripcion && (
                    <section className="form-section">
                      <h3>Descripción</h3>
                      <p className="muted">{viaje.descripcion}</p>
                    </section>
                  )}
                </section>
              </>
      ...
      <ConfirmModal
        isOpen={modalEliminacionAbierto}
        title="Eliminar viaje"
        message={`¿Deseas eliminar “${viaje?.nombre ?? 'este viaje'}”? Esta acción no se puede deshacer.`}
